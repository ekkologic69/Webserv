...
std::string Response::getContentType(const char* path)
{
    ...
    if (last_dot) 
    {
        ...
        // if (strcmp(last_dot, ".php") == 0) return "Content-Type: application/x-httpd-php\r\n";
    }
...
void	Response::RemoveDirectory(std::string &str)
{
	...
	if (directory)
	{
		...
		while ((content = readdir(directory)) != NULL)
		{
			if (content->d_name[0] == '.')
				continue;
			std::string temp = str + '/' + content->d_name;
	
			if (IsDirectory(temp.c_str()))
			{
				RemoveDirectory(temp);
				response = errorResponse[204];
...
void Response::DeleteRespond(Server &server)
{
	...
	else if (S_ISDIR(st.st_mode))
	{
		...
		else if (DirectoryAccess(path.c_str()))
		{
